...
window.notificationHelper = {
    ...
    // Update notification with new line
    updateNotification: async function(activityId, lineIndex) {
        const state = await this._getState(activityId);
        if (!state || !state.notesLines || lineIndex < 0 || lineIndex >= state.notesLines.length) {
            return;
        }
        
        const notesLines = state.notesLines;
        const currentLine = notesLines[lineIndex];
        
        // Update stored state
        await this._storeState(activityId, lineIndex, notesLines);
    ...
    // Close notification
    closeNotification: async function(activityId) {
        try {
            const registration = await this._getRegistration();
            const notifications = await registration.getNotifications({ tag: `guided-${activityId}` });
